 ...
 */
internal object NodeCliResolver {
    ...
    fun resolve(
        ...
        listChildren: (String) -> List<String> = ::defaultListChildren
    ): ResolvedCommand {
        // Windows inherits the registry PATH and runs through cmd.exe, which is always found.
        if (command.isEmpty() || isWindows) return ResolvedCommand(command, null)

        val binDirs = nodeBinDirs(userHome, env, listChildren)
        val program = command.first()
        val resolvedDir = if (program.contains('/')) {
            null
        } else {
            binDirs.firstOrNull { dir -> isExecutable("$dir/$program") }
        }

        val resolvedCommand =
            if (resolvedDir != null) listOf("$resolvedDir/$program") + command.drop(1) else command
        val pathDirs = listOfNotNull(resolvedDir) + binDirs
        return ResolvedCommand(resolvedCommand, buildPath(pathDirs, env))
    }
    ...
    fun nodeBinDirs(
        ...
        env: (String) -> String? = { System.getenv(it) },
        listChildren: (String) -> List<String> = ::defaultListChildren,
        readFileLines: (String) -> List<String> = ::defaultReadFileLines
    ): List<String> {
    ...
    private fun defaultReadFileLines(path: String): List<String> =
        runCatching { File(path).takeIf { it.isFile }?.readLines().orEmpty() }.getOrDefault(emptyList())

    private fun buildPath(prependDirs: List<String>, env: (String) -> String?): String {
        val ordered = LinkedHashSet<String>()
        prependDirs.forEach { if (it.isNotBlank()) ordered.add(it) }
        env("PATH").orEmpty().split(File.pathSeparatorChar).forEach {
            if (it.isNotBlank()) ordered.add(it)
        }
        return ordered.joinToString(File.pathSeparator)
    }

    private fun defaultListChildren(dir: String): List<String> =
        File(dir).listFiles()?.map { it.name }.orEmpty()

...
| 패키지 | 담는 것 |
|---|---|
| `agent` | 에이전트 계약, 4종 기록, 목록, 설치 명령, 상태 폴더 계산 |
...
| `bridge` | 요청 해석, 인증, 출처 판정, 자원 경로, 메시지 봉투, 웹 화면 주소 |
...
| `bridge` | HTTP 서버, 세션 보관, 이벤트 스트림, 라우트 실행 |
| `ide` | 편집기와 파일 접근, 선택 영역 수집, 열린 파일 추적, 끌어다 놓기 |
| `config` | 설정 저장 서비스와 설정 화면 |
| `process` | 직접 띄운 에이전트와 터미널 위젯 안 에이전트의 프로세스 창구 |
...
`platform`이지만 테스트하는 것이 네 가지 있습니다. 모두 무거운 환경 없이 돌고, 실패하면 사용자가 바로
겪습니다. 정적 자원 서빙, 브리지 인증과 메시지 주고받기, 선택 영역 수집, 단축키 우선순위입니다.

...
`com.intellij.ui.jcef`에 있는 내장 브라우저 클래스는 2026.1까지 플랫폼 코어에 있었습니다. 2026.2부터는
함께 배포되는 별도 플러그인으로 빠졌습니다. 그래서 이 클래스를 의존성으로 적지 않은 플러그인은 아예
불러올 수 없습니다.

| 값 | 유지해야 하는 것 |
|---|---|
| 의존성 | `plugin.xml`에 optional로 적고 `config-file="com.agentellij-jcef.xml"`을 지정한 `com.intellij.modules.jcef` |
| 추가 설명 파일 | 조건부 등록이 없는 `META-INF/com.agentellij-jcef.xml` |
| 빌드 의존성 | `bundledPlugin("com.intellij.modules.jcef")`. 2026.2 이상에서만 해석됩니다 |

의존성을 optional로 둔 이유는 2025.1과 2025.2가 이 식별자를 선언한 적이 없기 때문입니다. 필수로 적으면
그 판에서 플러그인이 로딩되지 않고, 아예 빼면 2026.2에서 그래픽 모드가 브라우저를 얻지 못합니다. 어느
쪽이든 그래픽 모드는 자기 실행 시점 확인을 그대로 갖고 있으므로, 브라우저가 없는 런타임에서는 실패가
아니라 안내 화면이 나옵니다.

추가 설명 파일은 비워 둡니다. 2025.1과 2025.2에는 의존성 식별자가 없지만 브라우저 클래스는 플랫폼에
남아 있으므로, 모든 확장 등록을 이 버전에서도 불러와야 합니다.

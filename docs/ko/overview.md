...
### 에이전트가 사용자를 기다릴 때 알리기

OpenCode, Claude Code, Codex가 메인 에이전트 턴을 끝내거나 구조화된 질문 UI를 열면 AgentellIJ가
IntelliJ 풍선과 운영체제 팝업을 함께 띄웁니다. IntelliJ가 활성 상태여도 둘 다 표시합니다. OS 팝업은
실행 중인 IntelliJ 플랫폼이 이미 제공하는 네이티브 알림 구현을 사용하므로 별도 알림 프로그램이 필요하지
않습니다.

OpenCode 웹 화면은 기존 `session.idle`과 `question.asked` 이벤트를 전달합니다. 터미널 화면은 각
에이전트가 지원하는 연동 지점을 씁니다. Codex는 완료 알림 명령과 `request_user_input` 수명 훅,
Claude Code는 `Stop`과 `AskUserQuestion` 훅, OpenCode는 실행 중 플러그인 이벤트를 사용합니다.
서브에이전트 완료 이벤트는 알림 조건이 아닙니다.

어댑터는 AgentellIJ 터미널 세션 안에서만 활성화됩니다. 직접 띄운 TUI는 어댑터를 명시적으로 거치고,
Terminal 화면은 지원 에이전트 래퍼를 그 셸의 검색 경로 앞에 둡니다. OpenCode 래퍼는 인라인 실행
설정으로 플러그인도 더합니다. 사용자 범위의 에이전트 설정은 고치지 않습니다. 화면을 닫거나 바꾸면
인증된 알림 주소도 무효가 됩니다. Codex의 생성된 수명 훅은 `/hooks`에서 사용자가 검토하고 신뢰해야
합니다. 완료 알림은 이 일회성 훅 신뢰와 관계없이 동작합니다.

완료 신호는 에이전트 턴이 끝났다는 뜻이지, 요청한 일이 모두 성공했다는 뜻은 아닙니다. 막힘 보고나 오류
응답도 턴을 끝낼 수 있습니다. 질문 신호는 대화 문구가 아니라 에이전트의 구조화된 질문 이벤트에서
옵니다. 플러그인은 이 수명 신호만 전달하며 에이전트의 판단을 소유하지 않습니다.

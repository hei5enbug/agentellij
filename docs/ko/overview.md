...
OpenCode, Claude Code, Codex가 에이전트 턴을 끝내면 AgentellIJ가 IntelliJ 알림을 띄웁니다. OpenCode 웹
화면은 기존 `session.idle` 이벤트를 전달합니다. 터미널 화면은 각 에이전트가 지원하는 연동 지점을
씁니다. Codex는 완료 알림 명령, Claude Code는 `Stop` 훅, OpenCode는 실행 중 플러그인 이벤트입니다.
...
이 알림은 에이전트 턴이 끝났다는 뜻이지, 요청한 일이 모두 성공했다는 뜻은 아닙니다. 마지막 질문, 막힘
보고, 오류 응답도 턴을 끝냅니다. 플러그인은 이 수명 신호만 전달하며 대화 문구를 해석하거나 에이전트의
판단을 소유하지 않습니다.
